...
export default function Home() {
  ...
  const addItem = async (item, imageUrl = null) => {
    ...
    if (docSnap.exists()) {
      ...
      await setDoc(docRef, { quantity: quantity + 1, imageUrl: imageUrl || existingImageUrl }, { merge: true });
    } else {
  ...
  const handleCapture = (imageUrl) => {
    const itemName = prompt("Enter the name of the pantry item:");
    if (itemName) {
  ...
  return (
    ...
    >
      ...
      <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center" alignSelf="center" margin="50px" gap={2}>
        ...
        <Box color='#f6f6f6' sx={{ fontFamily: 'Baskervville SC, serif' }} display="flex" flexDirection="column" alignItems="center" justifyContent="center">
          <p>This is a pantry tracker that supports image upload and storage.</p>
          <p>Designed by: Bri Kirchgessner</p>
          </Box>
